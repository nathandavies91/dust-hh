...
class Parser
{
     ...
     */
    public function __construct(?ParserOptions $options = NULL): void {
        if($options === NULL)
     ...
     */
    public function error(string $message, ParserContext $ctx) {
        // Find line and col
     ...
     * @return bool
     */
    public function isComment(string $str, int $offset): bool {
     ...
     * @return bool
     */
    public function isTag(string $str, int $offset): bool {
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parse(string $str): ?Ast\Body {
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseBodies(ParserContext $ctx): array {
        $lists = [];
        while (true)
        {
            if ($ctx->peek() != "{")
     ...
     * @return \Dust\Ast\Body|null
     */
    public function parseBody(ParserContext $ctx): ?Ast\Body {
     ...
     */
    public function parseBuffer(ParserContext $ctx): ?Ast\Buffer {
        // Some speedup here...
        $eol = strpos($ctx->str, "\n", $ctx->offset);
        if($eol === false)
            $eol = strlen($ctx->str);
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseContext(ParserContext $ctx): ?Ast\Context {
        if ($ctx->peek() != ":")
            return NULL;
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseFilters(ParserContext $ctx): array {
        $filters = [];
        while (true)
        {
            if ($ctx->peek() != "|")
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseIdentifier(ParserContext $ctx, bool $couldHaveNumber = false): ?Ast\Identifier {
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseInline(ParserContext $ctx): ?Ast\Inline {
        if ($ctx->peek() != "\"")
            return NULL;
     ...
     * @return null|string
     */
    public function parseInteger(ParserContext $ctx): ?string {
        $str = "";
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseLiteral(ParserContext $ctx): ?Ast\InlineLiteral {
     ...
     * @return null|string
     */
    public function parseKey(ParserContext $ctx): ?string {
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseNumber(ParserContext $ctx): ?string {
     ...
     * @return array
     */
    public function parseParameters(ParserContext $ctx): array {
        $params = [];
     ...
     * @return \Dust\Ast\Ast
     */
    public function parsePart(ParserContext $ctx): ?Ast\Ast {
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parsePartial(ParserContext $ctx): ?Ast\Partial {
        if ($ctx->peek() != "{")
            return NULL;
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseReference(ParserContext $ctx): ?Ast\Reference {
        if ($ctx->peek() != "{")
            return NULL;
     ...
     * @throws \Dust\Parse\ParseException
     */
    public function parseSection(ParserContext $ctx): ?Ast\Section {
